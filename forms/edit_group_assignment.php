  ...
  <body>
    ...
    if($role == 0){
      if (isset($_POST['selectGroup'])){
        $slID = $_POST['txtGroupName'];
        $query1 = "SELECT * FROM group_assignments WHERE group_id ='".$slID."'";
        $rs1 = mysql_query($query1) or die("Query to get insert new page failed " . mysql_error());
        echo'
        <div class="modal-content well">
          <form action="edit_group_assignment_message.php" method="POST" role="form">
          <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
            <input type="hidden" name="groupID" id="groupID" class="form-control" value="'.$slID.'">
            <legend>Member in this group</legend>
            <div class="form-group">
              <table>
                <tbody>';
                  while ($row_group = mysql_fetch_array($rs1)) {
                    $gUID2 = $row_group['user_id'];
                    $query2 = "SELECT display_name from users WHERE id ='".$gUID2."'";
                      ...
                      echo '
                          <tr>
                            <td class = "well"><div class="checkbox">
                                  <label>
                                    <input type="checkbox" name="inMembers[]" value="'.$gUID2.'" checked>
                                    '.$displayName2.'
                                  </label>
                            </td>
                          </tr>
          ...
          <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
            <legend>Member not in this group</legend>
            <div class="form-group">
              <table>
                <tbody>';
                  $query3 = "SELECT * from users WHERE id NOT IN (SELECT user_id FROM group_assignments WHERE group_id ='".$slID."')";
                  ...
                  while ($row_user3 = mysql_fetch_array($rs3)) {
                    ...
                    <tr>
                      <td class = "well"><div class="checkbox">
                        <label>
                        <input type="checkbox" name="outMembers[]" value="'.$gUID3.'">
                        '.$displayName3.'
                      </label>
                    </td>
                  </tr>';
            ...
            <div class="row">
              <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
              <center>
                <button type="submit" name="editGroupAs" id ="editGroupAs" class="btn btn-primary">Save</button>
              </center>
            </div>
            </div>
