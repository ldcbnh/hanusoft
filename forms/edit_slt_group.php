    ...
    <div class="container">
      ...
      if($role == 0){
        if(isset($_POST['selectGroup'])){
          $group_ID = $_POST['radioGroup'];
          $querySlinfor = "SELECT * FROM groups WHERE id ='".$group_ID."'";
          $rs = mysql_query($querySlinfor) or die("Query to get insert new page failed " . mysql_error());
          while ($row_group = mysql_fetch_array($rs)) {
            $groupName = $row_group['name'];
            $groupDescription = $row_group['description'];
          }
          echo '
              <div class="modal-content well">
                <div class="modal-header">
                  <h4 class="modal-title">Enter Group Information</h4>
                </div>
                  <form action="edit_group_message.php" enctype="multipart/form-data" method="POST" role="form">
                    <input type="hidden" name="gID" id="GID" class="form-control" value="'.$group_ID.'">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 form-group float-label-control">
                      <label for="txtName">Name</label>
                      <input type="text" class="form-control" name="txtName" id="txtName" required="required" placeholder="Group Name" value ="'.$groupName.'">
                    </div>  
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 form-group float-label-control">
                      <label for="txtDescription">Description</label>
                      <input type="text" class="form-control" name="txtDescription" id="txtDescription" required="required" placeholder="Description" value ="'.$groupDescription.'">
                    </div>  
                    

                    <div class="modal-footer">
                      <div style="float: right;">
                        <button type="submit" id="editGroup" name="editGroup" class="btn btn-primary">Save</button>
                      </div>
                    </div>
                  </form>
              </div>> ';
